...
## .gitignore の拡張子付きファイル名は Windows では大文字小文字を無視して当たる
> 昇格: 済 → lessons-global.md「環境」

- `config.ps1` を無視する指定が `scripts/lib/Config.ps1` にも当たり、読み込み処理が一度も commit されないまま push した。
  手元では動くので気付かず、ユーザーの「まだ残っている」という指摘で見直して発見した。
- push 前に「除外すべきものが入っていない」ことしか確認していなかった。**「必要なものが除外されていない」確認と、
  別ディレクトリに clone して動かす確認**をしていれば防げた。
- 対処: 無視パターンをパス付き（`/config/config.ps1`）に固定、読み込み処理を `LoadConfig.ps1` に改名して名前の衝突自体をなくした。
